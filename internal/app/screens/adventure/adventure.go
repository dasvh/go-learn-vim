...
type Adventure struct {
	...
	gameModeInfo     views.GameModeInfo
	gameStats        views.GameStats
	gameInstructions views.Instructions
	gameView         GameView
...
func LoadAdventure(repo storage.AdventureGameRepository, state storage.AdventureGameState, size tea.WindowSizeMsg) (*Adventure, error) {
	...
	adventure.gridWidth = size.Width - style.GetComponentWidth(style.Styles.Adventure.Map.Border)
	adventure.gridHeight = size.Height - (style.GetComponentHeight(style.Styles.Adventure.Header.Border) +
		style.GetComponentHeight(style.Styles.Adventure.Instructions.Style) +
		style.GetComponentHeight(style.Styles.Adventure.Map.Border))
...
func (a *Adventure) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		if msg.Width > 0 && msg.Height > 0 {
			a.size = msg
			a.gridWidth = msg.Width - style.GetComponentWidth(style.Styles.Adventure.Map.Border)
			a.gridHeight = msg.Height - (style.GetComponentHeight(style.Styles.Adventure.Header.Border) +
				style.GetComponentHeight(style.Styles.Adventure.Instructions.Style) +
				style.GetComponentHeight(style.Styles.Adventure.Map.Border))
